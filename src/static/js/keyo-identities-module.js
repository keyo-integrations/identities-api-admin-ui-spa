class KeyoIdentitiesModule extends HTMLElement {
  constructor() {
    ...
    this.cssCache = null; // Cache for loaded CSS
  }
  ...
  async loadStyles() {
    // Clear cache on each load to pick up CSS changes during development
    if (this.cssCache && window.location.hostname === 'localhost') {
      this.cssCache = null;
    }
    if (this.cssCache) {
    ...
    try {
      // Add cache busting to ensure fresh CSS
      const response = await fetch(`/static/css/keyo-identities-module.css?t=${Date.now()}`);
      if (!response.ok) {
      ...
      // Parse CSS sections
      const sharedStart = cssText.indexOf('/* === SHARED_START === */');
      const sharedEnd = cssText.indexOf('/* === SHARED_END === */');
      const fallbackStart = cssText.indexOf('/* === FALLBACK_START === */');
      const fallbackEnd = cssText.indexOf('/* === FALLBACK_END === */');
      
      if (sharedStart === -1 || sharedEnd === -1 || fallbackStart === -1 || fallbackEnd === -1) {
        throw new Error('CSS markers not found');
      }
      
      const shared = cssText.substring(sharedStart + 28, sharedEnd).trim();
      const fallback = cssText.substring(fallbackStart + 30, fallbackEnd).trim();
      
      this.cssCache = { shared, fallback };
      return this.cssCache;
    } catch (error) {
  ...
  getStyles(useBootstrap) {
    // If CSS is cached, use it synchronously
    if (this.cssCache) {
      return this.cssCache.shared + (useBootstrap ? '' : this.cssCache.fallback);
    }
    
    // Otherwise, return empty and trigger async load
    // This will cause a re-render once CSS is loaded
    if (!this.cssLoading) {
    ...
    // Return minimal styles while loading
    return `:host { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #1f2937; display: block; width: 100%; }`;
  }
      ...
      <div class="ki-token-entry-fullscreen">
        <div class="ki-card">
          <div class="ki-heading">
            <div>
              <div class="ki-logo-container">
                ...
                <h1>Identities Admin Dashboard</h1>
              </div>
            </div>
          </div>
          <form class="ki-form" id="ki-auth-form">
            ...
            <button class="btn btn-primary ki-btn-large" type="submit">
      ...
      <div>
        ...
        <div class="ki-list-header">
          ...
          <div class="ki-actions">
            <button class="btn btn-primary ki-btn-large" data-action="navigate-to-create">Create Account</button>
            <button class="btn btn-outline-secondary ki-btn-large" data-action="refresh" ${this.loading ? 'disabled' : ''}>
  ...
  renderDeviceAdminView() {
    ...
    return `
      <div>
        ${header}
        <div class="ki-list-header">
          <button class="btn btn-outline-secondary ki-btn-large" data-action="navigate-to-create">← Back to Create</button>
          <button class="btn btn-outline-secondary ki-btn-large" data-action="navigate-to-list">← Back to List</button>
        </div>
    ...
    const accountInfoContent = `
      ...
      <div class="ki-account-actions">
        <div class="ki-account-actions-row">
          <button class="btn btn-outline-secondary ki-btn-large" data-action="panel-cancel" type="button">Cancel</button>
          <button class="btn btn-outline-secondary ki-btn-large" data-action="panel-enroll" type="button" ${!this.getAutoSelectedDeviceId() || this.enrollLoading ? 'disabled' : ''}>
            ${this.enrollLoading ? '<span class="spinner"></span>&nbsp;Enrolling' : 'Enroll'}
          </button>
        </div>
        <div class="ki-update-action-row">
          <button class="btn btn-primary ki-btn-large" data-action="modal-update" type="button" ${this.modalSaving ? 'disabled' : ''}>
  ...
  renderModal() {
    ...
    const accountInfoContent = `
      ...
      <div class="ki-account-actions">
        <div class="ki-account-actions-row">
          <button class="btn btn-outline-secondary ki-btn-large" data-action="modal-cancel" type="button">Cancel</button>
          <button class="btn btn-outline-secondary ki-btn-large" data-action="panel-enroll" type="button" ${!this.getAutoSelectedDeviceId() || this.enrollLoading ? 'disabled' : ''}>
            ${this.enrollLoading ? '<span class="spinner"></span>&nbsp;Enrolling' : 'Enroll'}
          </button>
        </div>
